...
class Users_prep_model extends Model {
...
WHERE `users_id` = '5'
 */
         $this->db->select("main.*, `list_origin_country`.`name` as origin_country_name, list_current_country.name as current_country_name, list_tipodoc.name as tipodoc_name");
         $this->db->join(TBL_LIST_COUNTRY." as list_origin_country", "list_origin_country.country_id = main.origin_country","left");
         $this->db->join(TBL_LIST_COUNTRY." as list_current_country","list_current_country.country_id = main.origin_country","left");
         $this->db->join(TBL_LIST_TIPODOC." as list_tipodoc","list_tipodoc.tipodoc_id = main.documento_tipo","left");
         $infoPrep = $this->db->get_where(TBL_USERS_PREPFIS." main", array('users_id' => $this->_users_id))->row_array();
...
//        $infoLang = $this->db->get_where(TBL_USERS_REPR_LANG, array('users_id' => $this->_users_id));
        ...
        $infoPrep['age'] = strlen($infoPrep['nacimiento'])<9?calc_age($infoPrep['nacimiento']):"0";
        $infoPrep['comboCurrentState']  = $this->lists_model->get_states($infoPrep['current_country'], array(""=>"Seleccione una Provincia"));

  //      $infoRepSports = $this->db->get_where(TBL_USERS_REPR_DEP,  array('users_id' => $this->_users_id));

        if( $infoPrep['origin_country']!=0 ) {

            $infoPrep['comboOriginState'] = $this->lists_model->get_states($infoPrep['origin_country'], array(""=>"Seleccione una Provincia"));
            $infoPrep['origin_state_select'] = $this->lists_model->get_states(false, null,$infoPrep['origin_state']);
            $infoPrep['current_state_select'] = $this->lists_model->get_states(false, null,$infoPrep['current_state']);
            
        }

        if( !empty($infoPrep['nacimiento']) ) $infoPrep['nacimiento'] = date('d-m-Y', $infoPrep['nacimiento']);
