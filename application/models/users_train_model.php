...
class Users_train_model extends Model {
...
WHERE `users_id` = '5'
 */
         $this->db->select(TBL_USERS_TRAINER.".*, `list_origin_country`.`name` as origin_country_name, list_current_country.name as current_country_name, list_tipodoc.name as tipodoc_name");
         $this->db->join(TBL_LIST_COUNTRY." as list_origin_country", "list_origin_country.country_id =".TBL_USERS_TRAINER. ".origin_country","left");
         $this->db->join(TBL_LIST_COUNTRY." as list_current_country","list_current_country.country_id =".TBL_USERS_TRAINER. ".origin_country","left");
         $this->db->join(TBL_LIST_TIPODOC." as list_tipodoc","list_tipodoc.tipodoc_id =".TBL_USERS_TRAINER. ".documento_tipo","left");
         $infoTrainer = $this->db->get_where(TBL_USERS_TRAINER, array('users_id' => $this->_users_id))->row_array();
...
//        $infoLang = $this->db->get_where(TBL_USERS_REPR_LANG, array('users_id' => $this->_users_id));
        ...
        $infoTrainer['age'] = strlen($infoTrainer['nacimiento'])<9?calc_age($infoTrainer['nacimiento']):"0";
        $infoTrainer['comboCurrentState']  = $this->lists_model->get_states($infoTrainer['current_country'], array(""=>"Seleccione una Provincia"));

  //      $infoRepSports = $this->db->get_where(TBL_USERS_REPR_DEP,  array('users_id' => $this->_users_id));

        if( $infoTrainer['origin_country']!=0 ) {

            $infoTrainer['comboOriginState'] = $this->lists_model->get_states($infoTrainer['origin_country'], array(""=>"Seleccione una Provincia"));
            $infoTrainer['origin_state_select'] = $this->lists_model->get_states(false, null,$infoTrainer['origin_state']);
            $infoTrainer['current_state_select'] = $this->lists_model->get_states(false, null,$infoTrainer['current_state']);
            
        }

        if( !empty($infoTrainer['nacimiento']) ) $infoTrainer['nacimiento'] = date('d-m-Y', $infoTrainer['nacimiento']);
     
        return array(
            'info_trainer'  => $infoTrainer,
//            'info_lang' => $infoLang,
            'info_pic' => $infoPic,
            'info_vid' => $infoVid,
